...
> 用途：用 BMAD v6 完整四阶段流程开发**正式新产品**（区别于 bmad-pilot 的 Quick Flow 试点）。
> 框架版本：BMAD v6.12.0（bmm + core 模块，官方 CLI 非交互安装）
> 配置：HTY / 中文沟通 / 中文文档输出
> 安装时间：2026-09-13
...
## 三、完整四阶段流程（你 = 评审决策方，AI = 执行方）

| 阶段 | 核心 skill（已验证存在） | 你要做的 | 产出物 |
|---|---|---|---|
| 1. 构思 | `bmad-brainstorming` / `bmad-product-brief` | 口述想法、纠偏、补领域知识 | BRIEF |
| 2. 需求 | `bmad-prd` | 评审 PRD（EARS 原则）、签字 | PRD.md |
| 3. 架构 | `bmad-architecture` | 确认技术边界/选型 | ARCH.md |
| 4. 拆分 | `bmad-create-epics-and-stories` | 确认故事优先级 | STORIES |
| 5. 构建 | `bmad-build` / `bmad-build-auto` | 验收代码 | 可运行代码 |
| 6. 测试 | `bmad-qa-generate-e2e-tests` | 确认用例覆盖 | E2E 测试 |
| 7. 复盘 | `bmad-retrospective` | 评审上线效果 | 复盘报告 |
